...
void TCPAssignment::packetArrived(std::string fromModule, Packet &&packet) {
  ...
  // check
  // printf("struct: got ?, s: %u, a: %u, from: %u, to: %u \n", ntohl(readtcphdr->th_seq), ntohl(readtcphdr->th_ack), ntohs(readtcphdr->th_sport), ntohs(readtcphdr->th_dport));
  // printf("each: got ?, s: %u, a: %u, from: %u, to: %u \n", ntohl(seqnum), ntohl(acknum), ntohs(srcport), ntohs(destport));
...
void TCPAssignment::syscall_connect(UUID syscallUUID, int pid, int fd, struct sockaddr* addr, socklen_t addrlen){
  ...
  // Select Src IP & Port
  sock -> sockaddrinfo.src_port = htons(rand() % 65535); // port num must be less than 65536
